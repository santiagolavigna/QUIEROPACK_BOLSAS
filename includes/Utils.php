...
class Utils{
    ...
    static function log($txt){
        ...
        $txt=$txt."----------------------------------------\n";
        $myfile = fopen("LOG.txt", "a") or die("ERROR UTILS!");
        fwrite($myfile, print_r($txt,true));
        fclose($myfile);
    }
    
       static function actualizaciones($txt){
        if(Utils::$stopLog) return null;
        $txt="\n".Utils::getFecha()."\n".$txt;
        $myfile = fopen("ACTUALIZACIONES.txt", "a") or die("ERROR UTILS!");
        fwrite($myfile, print_r($txt,true));
        fclose($myfile);
    }
    ...
    static function log_facturacion($txt){
        ...
        debug_backtrace(false))."\n".$txt;
        $txt=$txt."----------------------------------------\n";
        $myfile = fopen("LOG_FACTURACION.txt", "a") or die("ERROR UTILS!");
        fwrite($myfile, print_r($txt,true));
        fclose($myfile);
    }
    ...
    static function log_deleteds($txt){
        ...
        debug_backtrace(false))."\n".$txt;
        $txt=$txt."----------------------------------------\n";
        $myfile = fopen("LOG_DELETEDS.txt", "a") or die("ERROR UTILS!");
        fwrite($myfile, print_r($txt,true));
        fclose($myfile);
    }
