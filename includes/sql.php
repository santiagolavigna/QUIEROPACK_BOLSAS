...
function find_armadoBolsaList($table) {
  ...
  if(tableExists($table))
  {
    return find_by_sql("SELECT arm.id, arm.nombre, arm.centimetros,d.precio_armado_bolsa dolar, cast(( (arm.centimetros ) * 1000 * d.precio_armado_bolsa) /1000  as decimal(10,3)) precio FROM ".$db->escape($table)." arm INNER JOIN dolar d on 1 = 1");
  }
}
